...
from utils.connection import CM
...
if args['qp_type'] == e.IBV_QPT_UP and args['operation_type'] != e.IBV_WR_SEND:
...
if args['qp_type'] == e.IBV_QPT_UD:
    ...
    qa.qp_access_flags = e.IBV_ACCESS_REMOTE_WRITE | e.IBV_ACCESS_REMOTE_READ | e.IBV_ACCESS_LOCAL_WRITE
    if server:
        qp.to_rtr(qa)
    else:
        qp.to_rts(qa)
...
mr_size = args['size']
if server:
    if args['qp_type'] == e.IBV_QPT_UD:   # UD needs more space to store GRH when receiving.
        mr_size = mr_size + GRH_LENGTH
    content = 's' * mr_size
else:
    content = 'c' * mr_size

mr = MR(pd, mr_size, e.IBV_ACCESS_LOCAL_WRITE | e.IBV_ACCESS_REMOTE_WRITE | e.IBV_ACCESS_REMOTE_READ)
sgl = [SGE(mr.buf, mr.length, mr.lkey)]

if args['operation_type'] != e.IBV_WR_SEND:
    remote_info = conn.handshake(addr=mr.buf, rkey=mr.rkey)

def read_mr(mr):
    if args['qp_type'] == e.IBV_QPT_UD and server:
...
for i in range(args['iters']):
    ...
    if server and args['operation_type'] == e.IBV_WR_SEND:
    ...
    if not server:
        ...
        elif args['operation_type'] != e.IBV_WR_SEND:
    ...
    if not server or args['operation_type'] == e.IBV_WR_SEND:
